...
USE_KOMPOSE = ENV['USE_KOMPOSE'] || false
        ...
        kHost.trigger.after [:up] do
          ...
          if USE_KOMPOSE
            if OS.windows?
              ...
              #run_remote "sudo curl -s -L https://github.com/kubernetes-incubator/kompose/releases/download/v0.5.0/kompose-linux-amd64 -o /usr/bin/kompose"
              #run_remote "sudo chmod +x /usr/bin/kompose"
            else
              info "No Windows support"
            end
          end
